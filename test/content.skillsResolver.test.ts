...
describe("skill key helpers", () => {
    it("skillInstructionsKeyForMode builds folder key; normal maps to strong", () => {
        assert.strictEqual(
            ...
            "functions/extractTopics/strong"
        );
        assert.strictEqual(
        ...
        assert.strictEqual(
            ...
            "functions/extractTopics/weak"
        );
        assert.strictEqual(
            skillInstructionsKeyForMode("ai.judge.v1", "ultra"),
            "functions/ai.judge.v1/ultra"
        );
    });
    it("skillRulesKey builds rules key", () => {
        assert.strictEqual(skillRulesKey("extractTopics"), "functions/extractTopics/rules");
        assert.strictEqual(skillRulesKey("ai.judge.v1"), "functions/ai.judge.v1/rules");
    });
});
...
describe("getSkillInstructions / setSkillInstructions (folder-based)", () => {
    ...
    it("setSkillInstructions calls resolver.set with folder key (functions/<id>/strong)", async () => {
        ...
        assert.strictEqual(setKey, "functions/mySkill/strong");
...
describe("getSkillRules / setSkillRules (folder-based)", () => {
    ...
    it("setSkillRules calls resolver.set with folder key (functions/<id>/rules)", async () => {
        let setKey: string | null = null;
        let setContent: string | null = null;
        const mockResolver = {
            set: async (key: string, content: string) => {
        ...
        assert.strictEqual(setKey, "functions/mySkill/rules");
...
describe("getSkillNamesFromContent", () => {
    it("returns names from folder-based keys only (functions/<id>/...)", async () => {
        const mockResolver = {
            listKeys: async () => [
                "functions/foo/strong",
                "functions/foo/rules",
                "functions/bar/strong",
            ],
    ...
    it("derives names from folder-based keys (functions/<id>/weak, strong, rules)", async () => {
        const mockResolver = {
            listKeys: async () => ["functions/extractTopics/weak", "functions/extractTopics/strong", "functions/matchLists/weak"],
        } as Parameters<typeof getSkillNamesFromContent>[0];
        const names = await getSkillNamesFromContent(mockResolver);
    ...
    it("returns unique names when a skill has multiple keys", async () => {
        const mockResolver = {
            listKeys: async () => ["functions/foo/weak", "functions/foo/strong", "functions/foo/rules"],
        } as Parameters<typeof getSkillNamesFromContent>[0];
        const names = await getSkillNamesFromContent(mockResolver);
...
describe("version APIs (require nx-content getVersions, getAtRef, setActiveVersion)", () => {
    ...
    it("setSkillInstructionsActiveVersion calls resolver.setActiveVersion", async () => {
        ...
        assert.strictEqual(calledKey, "functions/mySkill/strong");
    ...
    it("setSkillRulesActiveVersion calls resolver.setActiveVersion", async () => {
        ...
        assert.strictEqual(calledKey, "functions/mySkill/rules");
    });
